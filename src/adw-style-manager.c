...
struct _AdwStyleManager
{
  ...
  GtkCssProvider *color_provider; //F1_PATCH 1: Added for backwards-compatability.
                   ...
                   StylesheetUpdateFlags  flags)
{
  ...
  bool base_is_main = false; // My addition
...
adw_style_manager_constructed (GObject *object)
{
  ...
  if (self->display) {
    ...
    if (!adw_is_granite_present () && !g_getenv ("GTK_THEME")) {
      ...
      self->color_provider = gtk_css_provider_new (); //F3_PATCH 1: added back color_provider for backwards compatability.
      gtk_style_context_add_provider_for_display (self->display,
                                                  GTK_STYLE_PROVIDER (self->color_provider),
                                                  GTK_STYLE_PROVIDER_PRIORITY_THEME);//END F3_PATCH 1

      self->accent_provider = gtk_css_provider_new ();
      gtk_style_context_add_provider_for_display (self->display,
                                                  GTK_STYLE_PROVIDER (self->accent_provider),
                                                  GTK_STYLE_PROVIDER_PRIORITY_FALLBACK); //PATCH 2: changed "THEME" to "FALLBACK" to allow overide of accent by the theme.
...
adw_style_manager_dispose (GObject *object)
{
  ...
  g_clear_object (&self->color_provider); // F4_PATCH 1: Added for bacwards-compatability
